...
app.post('/chat', async (req, res) => {
    ...
    try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1/models/gemini-1.5-pro:generateContent?key=${process.env.GEMINI_API_KEY}`, {
            ...
            body: JSON.stringify({
                contents: [
                    { role: 'user', parts: [{ text: "You are a highly skilled cognitive behavioral therapist. Respond empathetically and provide guidance based on CBT principles. Encourage self-reflection and offer actionable steps. Also, chat like a normal human being. Keep responses concise yet meaningful." }] },
                    ...chatHistory
                ]
            })
        });
